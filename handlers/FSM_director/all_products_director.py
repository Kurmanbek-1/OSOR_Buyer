...
import asyncpg
from keyboards.buttons import StartDirector
...
async def fsm_start(message: types.Message):
    if message.from_user.id in Director:
        CategoryButtonsDirector = ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=False,
        ...
        await message.answer(f"Категория товара?", reply_markup=CategoryButtonsDirector)
    else:
...
async def cancel_reg(message: types.Message, state: FSMContext):
    if message.from_user.id in Director:
        ...
        if current_state is not None:
            await state.finish()
            await message.answer('Отменено!', reply_markup=StartDirector)
            # await message.answer("Чтобы удалить товар нажмите на кнопку 'Удалить' под сообщением")
    else:
        await message.answer('Вы не директор!')
