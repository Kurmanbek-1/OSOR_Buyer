...
async def fsm_start(message: types.Message):
    if message.from_user.id in staff:
        ...
        await FSM_order.name_buyer.set()
        await message.answer("Байер?!", reply_markup=buttons.cancel_for_client)
...
async def load_arcticle(message: types.Message, state: FSMContext):
    ...
    await FSM_order.next()
    await message.answer('Ваш размер? (xxl, l и т.д)')
...
def register_order_for_client(dp: Dispatcher):
    ...
    dp.register_message_handler(load_name_buyer, state=FSM_order.name_buyer)
